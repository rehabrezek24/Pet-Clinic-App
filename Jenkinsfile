pipeline {
    ...
    stages {
		...
		sh """
			...
			docker run -d  -p 8070:8080  --name "java-app-dev"   --network net-1  -e SPRING.PROFILES.ACTIVE=postgres  -e POSTGRES_USER=petclinic \ 
			-e POSTGRES_URL=jdbc:postgresql://db-dev/petclinic  -e POSTGRES_PASSWORD=petclinic  petclinic:v$BUILD_NUMBER 
    		"""
            }
        }
	 ...
	 stage('Deploy on Prod')  {
		 ...
		 sh """
                	...
                	docker run -d -p 8090:8080 --name "java-app-prod"  --network net-1 -e SPRING.PROFILES.ACTIVE=postgres  -e POSTGRES_USER=petclinic \
			-e POSTGRES_URL=jdbc:postgresql://db-prod/petclinic  -e POSTGRES_PASSWORD=petclinic  petclinic:v$BUILD_NUMBER
		 """
            }
        }
    }
